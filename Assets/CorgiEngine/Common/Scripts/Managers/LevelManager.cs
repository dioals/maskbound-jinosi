...
namespace MoreMountains.CorgiEngine
{
	...
	public class LevelManager : MMSingleton<LevelManager>, MMEventListener<CorgiEngineEvent>
	{
		...
		protected virtual void Initialization()
		{
			...
			switch (CheckpointAttributionAxis)
			{
				case CheckpointsAxis.x:
					...
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderBy (o => o.transform.position.x).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderByDescending (o => o.transform.position.x).ToList ();
					}
					break;
				case CheckpointsAxis.y:
					if (CheckpointAttributionDirection == CheckpointDirections.Ascending)
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderBy (o => o.transform.position.y).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderByDescending (o => o.transform.position.y).ToList ();
					}
					break;
				case CheckpointsAxis.z:
					if (CheckpointAttributionDirection == CheckpointDirections.Ascending)
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderBy (o => o.transform.position.z).ToList ();
					}
					else
					{
						Checkpoints = FindObjectsByType<CheckPoint> (FindObjectsSortMode.None).OrderByDescending (o => o.transform.position.z).ToList ();
					}
					break;
		...
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerator LoadGameOverSceneCo()
		{
			yield return MMCoroutine.WaitFor(GameManager.Instance.DelayBeforeGameOverSceneLoad);
		...
		protected virtual void GenerateColliderBounds()
		{
			...
			if ((BoundsCollider2D == null) && (BoundsMode == BoundsModes.TwoD))
			{
				...
				_collider2D.compositeOperation = Collider2D.CompositeOperation.Merge;
